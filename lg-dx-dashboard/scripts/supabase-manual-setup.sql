-- LG DX Dashboard 데이터베이스 설정
-- Supabase 웹 대시보드 > SQL Editor에서 실행하세요

-- 1. UUID 확장 활성화 (이미 활성화되어 있을 수 있음)
CREATE EXTENSION IF NOT EXISTS "uuid-ossp";

-- 2. users 테이블 생성 (auth.users와 연동)
...
-- 5. daily_statistics 테이블 생성 (선택사항)
...
-- 완료 메시지
SELECT 'LG DX Dashboard database setup completed!' as message;